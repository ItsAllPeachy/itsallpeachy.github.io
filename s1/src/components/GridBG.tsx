///* Code shamelessly stolen from https://github.com/raexera/raexera.github.io/blob/main/src/layouts/BaseLayout.astro because thier site is amazing *///
...
export default function Boxes({ className = "" }: GridProps) {
  ...
  return (
    <div ref={containerRef} className={`absolute flex h-full w-full justify-center overflow-hidden bg-white dark:bg-black ${className}`} style={{ height: `${dimensions.height}px`, zIndex: -1 }}>
      ...
      <div className="absolute left-0 top-0 h-full w-1/2">
        ...
        <div className="flex h-full w-full flex-col divide-y divide-black/25 border-l border-t border-white dark:divide-white/25 dark:border-black">
          {Array.from({ length: rows }).map((_, rowIndex) => (
            <div key={rowIndex} className="relative flex w-full divide-x divide-black/25 dark:divide-white/25" style={{ height: `${cellSize}px` }}>
